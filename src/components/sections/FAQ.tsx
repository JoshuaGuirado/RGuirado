  ...
  return (
    <section className="py-32 bg-dark-900 relative overflow-hidden border-t border-white/5" aria-labelledby="faq-heading">
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10">
        <div id="faq-heading" className="text-center space-y-4 mb-20">
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => {
            ...
            return (
              <div 
                key={index} 
                className="border border-white/5 bg-dark-800 rounded-3xl overflow-hidden transition-all duration-300 hover:border-gold-500/30 shadow-lg"
              >
                <button
                  onClick={() => toggleFAQ(index)}
                  className="w-full flex items-center justify-between p-8 text-left cursor-pointer group select-none"
                  aria-expanded={isOpen}
                >
                  <div className="flex gap-4 items-center pr-4">
                    <HelpCircle className="w-5 h-5 text-gold-500 shrink-0" />
                    <span className="text-lg font-bold text-white group-hover:text-gold-400 transition-colors leading-relaxed">
                      {faq.q}
                    </span>
                  </div>
                  <ChevronDown 
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div className="px-8 pb-8 pt-2 text-gray-400 text-md leading-relaxed font-light border-t border-white/5 bg-dark-900/30">
                        {faq.a}
                      </div>
                    </motion.div>
                  )}
